...
import com.leightek.todo.domain.Person;
...
public class ToDoSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public ToDoSecurityConfig(RestTemplateBuilder restTemplateBuilder, ToDoProperties toDoProperties) {
        ...
        this.restTemplate = restTemplateBuilder.basicAuthentication(toDoProperties.getUsername(),  // basicAuthorization
            ...
            public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
                try {
                    builder = UriComponentsBuilder
                            .fromUriString(toDoProperties.getFindByEmailUri()).queryParam("email", username);
                    log.info("Querying: " + builder.toUriString());

                    ResponseEntity<EntityModel<Person>> responseEntity = restTemplate
                            .exchange(RequestEntity.get(URI.create(builder.toUriString()))
                                    .accept(MediaTypes.HAL_JSON)
                                    .build(), new ParameterizedTypeReference<EntityModel<Person>>() {
                            });
